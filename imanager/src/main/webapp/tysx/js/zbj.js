...
jQuery(document).ready(function ($) {
    ...
    $("#rightTab li").each(function(i,v){
        $(v).click(function(){
        	var liveMsgType = $(v).attr("liveMsgType");
            if(liveMsgType==2){
...
$("#wdcom .slimScrollDiv").css("width","100%")
$(".zbj-ansbtn").hide();
$(document).delegate(".answerCon","mouseenter",function(){
    $(this).find(".zbj-ansbtn").show();
});
$(document).delegate(".answerCon","mouseleave",function(){
    $(this).find(".zbj-ansbtn").hide();
});

$(document).delegate(".circleUserUl li input","blur",function(){
	$(this).attr("readonly","readonly");
});
$(document).delegate(".circleUserUl li input","click",function(){
            ...
            success: function (data) {
                ...
                if(data.status==1){
                    ...
                    for (var i = 0; i < fileList.length; i++) {
                        ...
                        console.log(imgUrl)
                        imgArr.push(imgUrl);
                        var html = "<li class=\"addGuests\"><div class='circleUserPic'><img src='"+httpUrl+"'/></div><input type='text'><i class='fa fa-close'></i></li>"
                        $(".circleUserUl").append(html)
                        var addPic = $(".addPic").clone();
                        $(".addPic").remove();
                        $(".circleUserUl").append(addPic);
                    };
                }else{
                    alert("上传失败");
                }
...
$(document).delegate(".addPic input","change",function(){
    imgChange();
});
$(".circleUserBox").slimScroll({
    height:"230px"
})

$(document).delegate(".circleUserUl li","mouseenter",function() {
    $(this).find("i").css("display","block")
})
$(document).delegate(".circleUserUl li","mouseleave",function() {
