(function() {
	$(document).ready(function() {
		...
		$("#new-event").live('submit', function(e) {
			...
			if (value.length > 0) {
				$("#events .box-content")
						.prepend("<div class='label label-important external-event'>"
								+ value + "</div>");
				$("#new-event-input").val("");
		...
		$("#slider-example1").slider({
					...
					slide : function(event, ui) {
						return $("#slider-example1-amount")
								.text("$" + ui.value);
					}
				});
		$("#slider-example1-amount").text("$"
				+ $("#slider-example1").slider("value"));
		$("#slider-example2").slider({
			...
			values : [75, 300],
			slide : function(event, ui) {
				return $("#slider-example2-amount").text("$" + ui.values[0]
						+ " - $" + ui.values[1]);
			}
		});
		$("#slider-example2-amount").text("$"
				+ $("#slider-example2").slider("values", 0) + " - $"
				+ $("#slider-example2").slider("values", 1));
		...
		$(".todo-list .new-todo").live('submit', function(e) {
			...
			if (todo_name.length !== 0) {
				li = $(this).parents(".todo-list").find("li.item").first()
						.clone();
		...
		$("#comments-more-activity").click(function(e) {
			...
			setTimeout((function() {
						...
						list = $("#comments-more-activity").parent().parent()
								.find("ul");
						list.append(list.find("li:not(:first)").clone().effect(
								"highlight", {}, 500));
		...
		$("#users-more-activity").click(function(e) {
			...
			setTimeout((function() {
						...
						list = $("#users-more-activity").parent().parent()
								.find("ul");
						list.append(list.find("li:not(:first)").clone().effect(
								"highlight", {}, 500));
		...
		$(".chat .new-message").live('submit', function(e) {
			...
			if (message.length !== 0) {
				...
				timeago.attr("title", "" + (date.getFullYear()) + "-"
								+ (date.getDate()) + "-"
								+ (date.getMonth() + 1) + " "
								+ (date.getHours()) + ":" + (date.getMinutes())
								+ ":" + (date.getSeconds()) + " +0200");
				timeago.text("" + months[date.getMonth()] + " "
				...
				li.effect("highlight", {}, 500);
				reply = scrollable.find("li")
						.not(":contains('" + sender + "')").first().clone();
				setTimeout((function() {
							date = new Date();
							...
							$(scrollable).slimScroll({
								scrollTo : scrollable.prop('scrollHeight')
										+ "px"
							});
		...
		$(".full-calendar-demo").fullCalendar({
			...
			eventClick : function(calEvent, jsEvent, view) {
				return bootbox.confirm(
						"Do you really want to delete selected event?",
						function(result) {
							if (result === true) {
								return $(".full-calendar-demo").fullCalendar(
			...
			drop : function(date, allDay) {
				...
				$(".full-calendar-demo").fullCalendar("renderEvent",
		...
		$("#daterange2").daterangepicker({
					...
					return $("#daterange2").parent().find("input").first()
							.val(start.format("MMMM D, YYYY") + " - "
		...
		$("#daterange").daterangepicker({
			...
			return $("#daterange span").html(start.format("MMMM D, YYYY")
					+ " - " + end.format("MMMM D, YYYY"));
		});
	...
	this.setTimeAgoExample = function(selector) {
		...
		months = ["January", "February", "March", "April", "May", "June",
				...
				"December"];
		timeago = selector;
		timeago.attr("title", "" + months[date.getMonth()] + " "
						+ (date.getDate()) + ", " + (date.getFullYear()) + " "
						+ (date.getHours()) + ":" + (date.getMinutes()));
		timeago.text("" + months[date.getMonth()] + " " + (date.getDate())
