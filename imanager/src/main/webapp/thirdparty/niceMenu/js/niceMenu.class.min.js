...
var niceMenu = {
	...
	initMenu : function() {
		var obj = this;
		$(option.element).css("height", h);
		$(option.element).first().find("ul").addClass("pos--1").css("height", h);
		$(option.element).first().find("li").each(function(index) {
			var subObj = $(this);
			subObj.find("ul").first().addClass("poz-" + index);
			if (subObj.find("ul").first().size() > 0) {
				subObj.find("a").first().css({}).addClass("haveSubItem");
				subObj.find("a").first().next("ul").css("opacity", 0).css("visibility", "hidden");
			}
		});
	},
	moveMenu : function(subObj, action) {
		var obj = this;
		if (!option.running) {
			option.running = true;
			if (action == "next") {
