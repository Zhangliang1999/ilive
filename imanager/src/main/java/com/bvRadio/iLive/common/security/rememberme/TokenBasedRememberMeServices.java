...
import com.bvRadio.iLive.common.security.userdetails.UserDetails;
...
public class TokenBasedRememberMeServices extends AbstractRememberMeServices {
	...
	protected UserDetails processAutoLoginCookie(String[] cookieTokens, HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			throw new InvalidCookieException("Cookie token[3] did not contain a valid number (contained '" + cookieTokens[3] + "')");
		}
		// Check the user exists.
		// Defer lookup until after expiry time checked, to possibly avoid
		// expensive database call.

		UserDetails user = getUserDetailsService().loadUser(userId, cookieTokens[0]);
		...
		String expectedTokenSignature = makeTokenSignature(tokenExpiryTime, user.getUsername(), user.getPassword(), user.getId());
	...
	protected String makeTokenSignature(long tokenExpiryTime, String username, String password, Long id) {
		return DigestUtils.md5Hex(username + ":" + tokenExpiryTime + ":" + password + ":" + getKey() + ":" + id);
	}
	...
	public boolean onLoginSuccess(HttpServletRequest request, HttpServletResponse response, UserDetails user) {
		...
		String signatureValue = makeTokenSignature(expiryTime, username, password, user.getId());

		setCookie(new String[] { username, Long.toString(expiryTime), signatureValue, user.getId().toString() }, tokenLifetime, request, response);
