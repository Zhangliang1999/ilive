		...
		link : function(scope, element, attrs, ngModel) {
			$timeout(function() {
				...
				var ajax_request = function() {
					var queryCondition = {
						queryContent : scope.ngModel
					}
					enterpriseService.list(queryCondition).then(
							function(response) {
								if (response.code == 1) {
									...
									$.each(result, function(index, bean) {
										...
										// 创建li标签,添加到下拉列表中
										$('<li data-id="' + enterpriseId + '"></li>').text(
												enterpriseName + "(" + enterpriseId + ")").appendTo($autocomplete).addClass(
											...
											ngModel.$setViewValue(enterpriseName + "(" + enterpriseId + ")");
											ngModel.$render();
											// 清空并隐藏下拉列表
											$autocomplete.empty().hide();
										});
