myapp.controller("enterpriseDetailAct", function($scope, $http, $state, queryCondition, enterpriseStatisticService) {
	$scope.queryCondition = queryCondition;
	// 加载数据
	enterpriseStatisticService.getTotalData($scope.queryCondition).then(function(response) {
		$scope.totalData = resolveBean(response);
	});
	enterpriseStatisticService.getMaxData($scope.queryCondition).then(function(response) {
	...
	$scope.getViewSourceData = function() {
		enterpriseStatisticService.getViewSourceData($scope.viewSourceQueryCondition).then(function(response) {
	...
	$scope.getViewTimeDataForLine = function() {
		enterpriseStatisticService.getViewTimeDataForLine($scope.viewTimeForLineQueryCondition).then(function(response) {
	...
	$scope.getViewTimeDataForColumn = function() {
		enterpriseStatisticService.getViewTimeDataForColumn($scope.viewTimeForColumnQueryCondition).then(function(response) {
	...
	$scope.getViewLocationData = function() {
		enterpriseStatisticService.getViewLocationData($scope.viewLocationQueryCondition).then(function(response) {
	...
	$scope.getViewTimeDataForPie = function() {
		enterpriseStatisticService.getViewTimeDataForPie($scope.viewTimeForPieQueryCondition).then(function(response) {
