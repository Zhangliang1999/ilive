...
myapp.filter("updateType", function() {
	return function(input, showStr) {
		if (input) {
			for (i = 0; i < updateTypeBeans.length; i++) {
				if (input == updateTypeBeans[i].value) {
					return updateTypeBeans[i].label;
				}
			}
		}
		return showStr;
...
myapp.filter("mediaFileType", function() {
	return function(input, showStr) {
		if (input) {
			for (i = 0; i < mediaFileTypeList.length; i++) {
				if (input == mediaFileTypeList[i].value) {
					return mediaFileTypeList[i].label;
				}
			}
		}
		return showStr;
