...
public class ILIvePageDecorateDaoImpl extends HibernateBaseDao<PageDecorate, Long> implements ILIvePageDecorateDao {
	...
	public void updatePageDecorate(PageDecorate page) {
		StringBuilder hql = new StringBuilder("update PageDecorate set menuName = ?,hrefValue = ?,menuName1 = ?,relatedVideo= ? ");
		...
		query.setParameter(2, page.getMenuName1());
		query.setParameter(3, page.getRelatedVideo());
	...
	public PageDecorate getPageDecorateByEventIdAndType(PageDecorate page) {
		String hql = "from PageDecorate where liveEventId = :liveEventId and menuType = :menuType";
		Query query = this.getSession().createQuery(hql);
		query.setParameter("liveEventId", page.getLiveEventId());
		query.setParameter("menuType", page.getMenuType());
		List<PageDecorate> list = query.list();
		if(list.size() ==0) {
			return null;
		}else {
