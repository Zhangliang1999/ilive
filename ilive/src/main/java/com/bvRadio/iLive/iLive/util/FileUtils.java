...
public class FileUtils {
	...
	public static String getRootPath(String path) {
		...
		String tempPath = classesPath.replace("/WEB-INF/classes", "/" + path);
		if (null != tempPath && tempPath.startsWith("file:")) {
	...
	/**
	 * 判断是否为允许上次的文件类型
	 * 根据.properties admin.allowedFileType属性
	 * @param ext
	 * @return
	 */
    public static boolean isAllowedFileType(String ext){
    	String[] allowedFileType = StringUtils.split(ConfigUtils.get("allowedFileType"), ",");
    	ext = ext.toLowerCase();
    	if( !Arrays.asList(allowedFileType).contains(ext)) {
    		return false;
    	}
    	return true;
    }
