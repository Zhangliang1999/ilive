...
public class BBSDiymodelDaoImpl extends HibernateBaseDao<BBSDiymodel, Integer> implements BBSDiymodelDao {
	...
	public List<BBSDiymodel> getListByDiyformIdAndMark(Integer diyformId, Integer mark) {
		...
		if (mark != null) {
			f.append(" and bean.updateMark = :updateMark");
			f.setParam("updateMark", mark);
		}
		f.append(" and bean.diymodelId in(select max(diymodelId) from BBSDiymodel group by diymodelType)");
		f.append(" order by bean.diyOrder asc");
		return find(f);
	}
