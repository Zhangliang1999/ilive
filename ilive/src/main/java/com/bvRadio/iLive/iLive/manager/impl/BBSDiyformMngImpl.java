...
public class BBSDiyformMngImpl implements BBSDiyformMng {
	...
	@Autowired
	private BBSDiyformNameService bbsDiyformNameService;
	...
	public BBSDiyform save(BBSDiyform bbsDiyform, List<BBSDiymodel> bbsDiymodelList) {
		...
		if (nextId != -1) {
			...
			bbsDiyform.setUpdateTime(new Timestamp(new Date().getTime()));
			bbsDiyform.setMark(0);
			dao.save(bbsDiyform);
			
			BBSDiyformName formName = new BBSDiyformName();
			formName.setDiyformName(bbsDiyform.getDiyformName());
			formName.setUpdateMark(0);
			formName.setDiyformId(nextId);
			bbsDiyformNameService.save(formName);
				...
				if (nextId != -1) {
					...
					for (int i = 0; i < bbsDiymodelList.size(); i++) {
						try {
							...
							bbsDiymodel.setUpdateMark(bbsDiyform.getMark());
...
//							}
	...
	public BBSDiyform update(BBSDiyform bbsDiyform, List<BBSDiymodel> bbsDiymodelList) {
		if (null != bbsDiyform && null != bbsDiyform.getDiyformId()) {
			bbsDiyform.setUpdateTime(new Timestamp(new Date().getTime()));
			bbsDiyform.setMark(bbsDiyform.getMark()==null?0:bbsDiyform.getMark()+1);
			dao.update(bbsDiyform);
			
			BBSDiyformName formName = new BBSDiyformName();
			formName.setDiyformName(bbsDiyform.getDiyformName());
			formName.setUpdateMark(bbsDiyform.getMark());
			formName.setDiyformId(bbsDiyform.getDiyformId());
			bbsDiyformNameService.save(formName);
	...
	@Override
	public List<BBSDiyform> getListByRoomId(Integer roomId) {
		return dao.getListByRoomId(roomId);
	}
