...
var questionnaire_num = 0;
var problemAnswers = "";
var questionnaireId = 0;//投票活动ID
...
function checklg(obj) {
    ...
    var MaxQuestionnairenum = $parent.data('maxquestionnairenum')
    var now = new Date();
    if (now.getTime() - evTimeStamp < 100) {
        return;
    }
    evTimeStamp = now;
    var ChooseQuestionnairenum = $parent.find($("input[type='checkbox']:checked")).length
    if (ChooseQuestionnairenum > MaxQuestionnairenum) {
...
function selectWenjuan() {
    ...
    $.ajax({
        type: "GET",//请求方式 get/post
        url: "http://" + tomcatBase + "/ilive/questionnaire/getQuestionnaire.jspx",
        dataType: "jsonp",
        jsonp: "callback",
        cache: false,
        data: {
