				...
				if(prize==undefined||prize==null||prize==""){
					...
					if(prizeStatus==2){
						...
						if(lotteryType==2){
							...
							HTML += "	</div>";
							HTML += "</div>";
							loadJsCss("js/appshake.js","js");
							$(".drawMask").attr("drawMask","1");
							...
							HTML += "		</div>";
							HTML += "	</div>";
							HTML += "	<p class=\"guize\" style=\"margin-top: 0.3rem;\">剩余<span id=\"choujiangNumber\">"+userNum+"</span>次机会 &nbsp;<a onclick=\"prizeList("+prize.id+")\">查看奖品</a></p>";
							HTML += "</div>";
							loadJsCss("js/appgoldeggs.js","js");
							$(".drawMask").attr("drawMask","0");
							...
							if(prizeLength<8){
								...
								HTML += "	</div>";
								HTML += "</div>";
								HTML += "<p class=\"guize\" style=\"margin-top: 0.2rem;\">剩余<span id=\"choujiangNumber\">"+userNum+"</span>次机会 &nbsp;<a onclick=\"prizeList("+prize.id+")\" >查看奖品</a></p>";
							}
						}
						$("#prizrContentDiv").html(HTML);
						if(lotteryType==3){
		...
		success : function(data) {
			...
			if(data.code==1){
				...
				var HTML  = "<div class=\"drawList\">";
					...
					HTML += "	</div>";
					HTML += "	<div class=\"ensure\" style=\"position:fixed;bottom:0;\"  onclick=\"inPrize()\">一&nbsp;键&nbsp;参&nbsp;与</div>";
					HTML += "</div>";
				$("#prizrContentDiv").html(HTML);
				$(".drawMask").slideDown(500);
			}else{
...
$(document).ready(function(){
    //点击打开抽奖遮罩层
    $("body").delegate(".drawIcon","click",function(){
    	//抽奖信息获取
    	var userId = $("input[name=userId]").val();//用户ID
		var roomId = $("input[name=roomId]").val();//直播间ID
		if(userId==0){
    ...
    //抽奖遮罩层点击关闭
    $(".drawClose .fa-close").click(function(){
