...
$(document).ready(function(){
    //点击打开投票遮罩层
	$("body").delegate(".toupiaoIcon","click",function(){
    	//抽奖信息获取
    ...
    //投票遮罩层点击关闭
    $(".voteClose").on('click', function(){
        $(".voteMask").slideUp(500);
        //$(".drawMask").slideUp(500);
        $(".livemask").click()

    })
    $(".voteendButton").click(function(){
        $(".voteMask").slideUp(500);
    })
    //执行答题结束操作
			...
			success : function(data) {
				...
				if(data.code==1){
					...
					for (var i = 0; i < problemList.length; i++) {
						var problem = problemList[i];
						HTML += "	<div class=\"voteList\">";
						HTML += "		<div class=\"voteQuestion\">"+problem.problemName+"</div>";
						var listOption = problem.listOption;
						for (var j = 0; j < listOption.length; j++) {
