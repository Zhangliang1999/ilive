...
let _height = $(window).height() - $('.menuBox').offset().top
...
if (Params.fileId == '') {
    ...
    if (isWeiXin()) {
        // 执行操作
        createAPI("http://" + tomcatBase + "/ilive/app/wx/app.jspx", {fileId: Params.fileId, roomId: Params.roomId})
            .then(function (res) {
                let status = res.loginWx;
                if (status == 1) {
                    window.stop();
...
function init (){
    checklogin(function () {
        ...
        guide(function (liveEvent) {
            let viewAuthorized = liveEvent.viewAuthorized;
            /*
            2.密码观看 3.付费
            */
            switch (viewAuthorized) {
                ...
                default:
                    Promise.all([
                        //当前文件
                        createAPI("http://" + tomcatBase + "/ilive/app/room/vod/fileinfo.jspx", {fileId: Params.fileId}),
                        ...
                        // 更多回看
                        createAPI("http://" + tomcatBase + "/ilive/app/room/getrelatedlist.jspx", {fileId: Params.fileId}),

                        //自定义分享
                        createAPI("http://" + tomcatBase + "/ilive/app/wx/sharefileinfo.jspx", {fileId: Params.fileId})


                    ]).then(function (arr) {
                        let [getMediaFile, getEnterprise, getFileList, shareinfo] = arr;

                        //获取文件
                        if (getMediaFile.code == 1) {

                            setInit(getMediaFile, shareinfo) //设置初始 shareinfo 分享带入
                            setEnterprise(getEnterprise)//设置企业
                            setFileList(getFileList)//相关视频
...
function checklogin(callback) {
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            if (res.code == 1) {
                let roomNeedLogin = JSON.parse(res.data).roomNeedLogin;
                if (roomNeedLogin === 1) {
...
function guide(callback) {
    $.ajax({
        ...
        success: function (res) {
            switch (res.code) {
                case 1:
                    let liveEvent = res.data.liveEvent;
                    callback(liveEvent)
                    break
                case 402:
                    window.location.href = "http://" + h5Base + "/phone/login.html?roomId=" + Params.roomId + "&fileId=" + Params.fileId + "&userId=" + Params.userId;
                    ...
                    //     }
                    //     window.location.href = "http://" + h5Base + "/phone/phoneNumber.html?roomId=" + Params.roomId + "&fileId=" + Params.fileId + "&userId=" + userId;
                    // } else {
                    //     layer.open({
...
//设置初始
function setInit(data, shareinfo) {
    if (data.data != null && data.data != undefined) {
        ...
        setVideo(mediaFile.filePath)//视频
...
function setEnterprise(data) {
    if (data.code == 1) {
        ...
        concernTotalTemp = setNum(concernTotal); //关注人数
        ...
        //查询多少个点赞
        createAPI("http://apizbt.tv189.net/ilive/register/MediaRegisterCount.jspx", {
            fileId: Params.fileId
        }).then(function (res) {
            if (res.code == 0) {
        ...
        $('#concernTotal').find('span').text(concernTotalTemp)//粉丝数量
...
function setFileList(data) {
    if (data.code === 1 && data.data.length > 0) {
        let dom = createReviewFile(data.data)
        ...
        window.showAllReview.onclick = function () {
            popup({
                title: '相关视频',
                content: show_all_review(data.data)
            })
...
function changeVideo(fileId) {
    window.location.href = "http://" + h5Base + "/phone/reviewNew.html?roomId=" + Params.roomId + "&fileId=" + fileId;
}
...
//加载文档
let fileDoc = null;
let popupDisable = false //取消多次弹层
let docArr = [];

function showFileDoc() {
    createAPI("http://" + tomcatBase + "/ilive/document/getByfileId.jspx", {fileId: Params.fileId})
        .then(function (res) {
            if (res.code == 1) {
...
function createFileDoc(res) {
    ...
    res.forEach(function (value) {
        let str = '<dl class="document document_' + value.document.id + '" data-docid="' + value.document.id + '">\n' +
            '     <dd>\n' +
...
function fileDocDetail(docId) {
    ...
    //获取详细文档
    let curDoc = $('.showPopupContent').find('.document_' + docId)
    ...
    let mescroll2 = new MeScroll("mescroll2", {
        down: {
            use: false,
        },
        up: {
...
function wendangJS(dom, documentPictures) {
    let str = '<div class="swiper-container " id="gallery_' + documentPictures[0].docId + '">\n'
    str += '<div class="swiper-wrapper">\n'
    for (let i = 0; i < documentPictures.length; i++) {
        let url = documentPictures[i].url;
    ...
    let thumbsSwiper = new Swiper('#thumbs_' + documentPictures[0].docId, {
        spaceBetween: 10,
        slidesPerView: 3,
    ...
    setTimeout(function () {
        ...
        $(".menuBox").find('li').on('click', function () {
            ...
            if (this.className == 'talkLi') {
                let talkli = document.querySelectorAll('.talkLi')[1]
                let offset = talkli.offsetTop + 1
...
//下拉刷新
function upCallback() {
    getListDataFromNet(page.num, page.size, page.fileId, function (curPageData) {
...
function getListDataFromNet(pageNum, pageSize, fileId, successCallback, errorCallback) {
    setTimeout(function () {
        try {
...
function saveComments() {
    let msgContent = $.trim($("input[name=msgContent]").val());//发送内容
...
function saveMediaFile() {
    createAPI("http://" + tomcatBase + "/ilive/register/registerMedia.jspx", {
        ...
        $('.dianzan').find('i').addClass('checked')
        let dianzanTag = $('#dianzan-tag')
        let tagNum = dianzanTag.text()
...
function desc(content) {
    let title = $('.liveTitle').html();
    let view = $('.play-num').html()
...
//点赞弹层
window.dianzan.onclick = function () {
    if ($(this).find('i').hasClass('checked')) return
    saveMediaFile()

}
//文档弹层
window.wendang.onclick = function () {
    let docId = $('.document').eq(0).data('docid')
    fileDocDetail(docId)
}
